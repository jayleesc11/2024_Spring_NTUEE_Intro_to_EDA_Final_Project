...
vector<vector<int>> OARST(vector<Vertex> &vs, vector<vector<int>> &adj_list)
{
    vector<edge>  A;
    vector<Vertex> outv;
    ...
    while(!A.empty())
    {
       ...
       // printf("down\n");
        ...
        //   printf("e2 v1 = (%lf,%lf) v2 = (%lf,%lf) v3 = (%lf,%lf)\n",v1.coord.first, v1.coord.second,v2.coord.first, v2.coord.second,v3.coord.first, v3.coord.second);
            ...
            if((A.size()==0)||max_s==-1 || ((v1.coord.first-v2.coord.first)*(v2.coord.first-v3.coord.first)>=0 && (v1.coord.second-v2.coord.second)*(v2.coord.second-v3.coord.second)>=0))
            {
            //    printf("case1\n");
               Vertex tmp(0,{0,0},0);
