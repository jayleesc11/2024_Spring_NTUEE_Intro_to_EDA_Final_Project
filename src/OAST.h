...
vector<vector<int>> OAST(const vector<Vertex>& vertexes, vector<vector<int>>& adj_list, float threshold, float alpha, const vector<vector<int>>& congestion_map, float cell_width, bool overflow_type)
{   
    //vertex coor (pair) pin 1 for pin 0 for obstacle
    //printf("size = %d, adj size = %d",vertexes.size(),adj_list.size());
    vector<vector<float>> weight(adj_list.size(), vector<float>());
    vector<vector<float>> overflow_cost(adj_list.size(), vector<float>());
    vector<vector<float>> cost(adj_list.size(), vector<float>());
    vector<int> prev(adj_list.size(), 0);
    vector<float> dist(adj_list.size(), 0);
    ...
    for(int i=0; i<adj_list.size(); i++)
    {
        for(int j=0; j<adj_list[i].size(); j++)
        {
            ...
            int c1x = vertexes[i].coord.first/cell_width;
            int c1y = vertexes[i].coord.second/cell_width;
            int c2x = vertexes[adj_list[i][j]].coord.first/cell_width;
            int c2y = vertexes[adj_list[i][j]].coord.second/cell_width;
            // printf("c1x = %d, c1y = %d, c2x = %d, c2y = %d\n",c1x,c1y,c2x,c2y);
            if(c1x>=c2x && c1y>=c2y)
            {
            ...
            if(overflow_type)
            {
                if(overflow1>overflow2)
                    overflow_cost[i].push_back(overflow2);
                else
                    overflow_cost[i].push_back(overflow1);
            }
            else
            {
                overflow_cost[i].push_back(overflow1+overflow2);
            }
            
            cost[i].push_back(overflow_cost[i][j]*alpha + weight[i][j]);
    ...
    for(int i=0; i<vertex_num; i++)
    {
        ...
        dijkstra(adj_list,cost,i,dist,prev);
