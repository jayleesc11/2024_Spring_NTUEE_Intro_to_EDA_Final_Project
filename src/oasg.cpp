...
void OASG::OASG_R1(const Vertex& source, const int sourceobstacleid) {
    ...
    // Sort candidates by y-coordinate in decreasing order. If two vertices have the same y-coordinate, sort them by x-coordinate in decreasing order.
    sort(candidates.begin(), candidates.end(), [&](const int a, const int b) {
        if(vplist[a].coord.second == vplist[b].coord.second) {
            return vplist[a].coord.first > vplist[b].coord.first;
        }
        return vplist[a].coord.second > vplist[b].coord.second;
    });

    // Add edges to the OASG by bounding box
    if(!candidates.empty()) {
        if(candidates[0] == source.id) {
            return;
        } else {
            adjList[source.id].emplace_back(candidates[0]);
            adjList[candidates[0]].emplace_back(source.id);
        }
    }
    for(int c = 1; c < candidates.size(); c++) {
        ...
        bool add = true;
        for(int o = c - 1; o != -1; o--) {
            int oth = candidates[o];
            if(oth != cand && 
                // xc <= xo <= x, yc <= yo <= y
                (vplist[cand].coord.first <= vplist[oth].coord.first && vplist[oth].coord.first <= source.coord.first) &&
                (vplist[cand].coord.second <= vplist[oth].coord.second && vplist[oth].coord.second <= source.coord.second)) {
                add = false;
                break;
            }
        }
        if(add) {
            for(const auto& edge : adjList[source.id]) {
                if(edge == cand) {
                    add = false;
                    break;
                }
        ...
        if(add) {
            adjList[source.id].emplace_back(cand);
            adjList[cand].emplace_back(source.id);
        }
    }
}
...
void OASG::OASG_R2(const Vertex& source, const int sourceobstacleid) {
    ...
    // Sort candidates by y-coordinate in increasing order. If two vertices have the same y-coordinate, sort them by x-coordinate in decreasing order.
    sort(candidates.begin(), candidates.end(), [&](const int a, const int b) {
        if(vplist[a].coord.second == vplist[b].coord.second) {
            return vplist[a].coord.first > vplist[b].coord.first;
        }
    ...
    for(int c = 1; c < candidates.size(); c++) {
        ...
        for(int o = c - 1; o != -1; o--) {
            ...
            if(oth != cand && 
                // xc <= xo <= x, y <= yo <= yc
                (vplist[cand].coord.first <= vplist[oth].coord.first && vplist[oth].coord.first <= source.coord.first) &&
                (source.coord.second <= vplist[oth].coord.second && vplist[oth].coord.second <= vplist[cand].coord.second)) {
                add = false;
                break;
            }
        }
...
void OASG::OASG_R3(const Vertex& source, const int sourceobstacleid) {
    ...
    // Sort candidates by y-coordinate in increasing order. If two vertices have the same y-coordinate, sort them by x-coordinate in increasing order.
    sort(candidates.begin(), candidates.end(), [&](const int a, const int b) {
        if(vplist[a].coord.second == vplist[b].coord.second) {
            return vplist[a].coord.first < vplist[b].coord.first;
        }
    ...
    for(int c = 1; c < candidates.size(); c++) {
        ...
        for(int o = c - 1; o != -1; o--) {
            ...
            if(oth != cand && 
                // x <= xo <= xc, y <= yo <= yc
                (source.coord.first <= vplist[oth].coord.first && vplist[oth].coord.first <= vplist[cand].coord.first) &&
                (source.coord.second <= vplist[oth].coord.second && vplist[oth].coord.second <= vplist[cand].coord.second)) {
                add = false;
                break;
            }
...
void OASG::OASG_R4(const Vertex& source, const int sourceobstacleid) {
    ...
    // Sort candidates by y-coordinate in decreasing order. If two vertices have the same y-coordinate, sort them by x-coordinate in increasing order.
    sort(candidates.begin(), candidates.end(), [&](const int a, const int b) {
        if(vplist[a].coord.second == vplist[b].coord.second) {
            return vplist[a].coord.first < vplist[b].coord.first;
        }
